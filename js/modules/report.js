...
window.ReportMod = (function() {
  ...
  function render() {
    ...
    const html = `
      <div class="page-header">
        <div>
          <h2><span class="emoji">${ic('doc')}</span>看房报告</h2>
          <p class="page-desc">汇总房源记录、购房期望、看房计划与进度，一键导出 Word 文档</p>
        </div>
        <div class="page-actions">
          <button class="btn btn-primary btn-sm" onclick="ReportMod.exportWord()">${ic('download',15)} 导出 Word</button>
        </div>
      </div>
  ...
  function buildReportHTML(exp, records, plans, wf) {
    ...
    return `
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12.5px;margin-bottom:14px;">
        ...
        <tr><th style="border:1px solid var(--border-light);padding:6px;text-align:left;">房型</th><td style="border:1px solid var(--border-light);padding:6px;">${(exp.roomsNeeded?.bedrooms||3)}室${(exp.roomsNeeded?.livingRooms||2)}厅${(exp.roomsNeeded?.bathrooms||1)}卫 · ${exp.areaMin||'-'}~${exp.areaMax||'-'}㎡</td><th style="border:1px solid var(--border-light);padding:6px;text-align:left;">意向区域</th><td style="border:1px solid var(--border-light);padding:6px;">${Store.formatPreferredAreas()}</td></tr>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:14px;">
        ...
        <tbody>${plans.map(p=>`<tr><td style="border:1px solid var(--border-light);padding:5px;">${p.date||'-'}</td><td style="border:1px solid var(--border-light);padding:5px;">${p.district||'-'}</td><td style="border:1px solid var(--border-light);padding:5px;">${(p.targets||[]).join('、')||'-'}</td><td style="border:1px solid var(--border-light);padding:5px;">${p.status==='done'?'✓已看':(p.status==='expired'?'已过期':'待看')}</td></tr>`).join('')}</tbody>
  ...
  function exportWord() {
    ...
    const fullHTML = `<!DOCTYPE html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"><title>看房总报告</title>
      <style>
        body{font-family:'PingFang SC','Microsoft YaHei',sans-serif;font-size:12px;color:#1D1D1F;line-height:1.6;}
        h1{font-size:22px;color:#0071E3;text-align:center;border-bottom:2px solid #0071E3;padding-bottom:8px;}
        h2{font-size:15px;color:#0071E3;border-left:4px solid #0071E3;padding-left:8px;margin:20px 0 8px;}
  ...
  function exportHTML() {
    ...
    const fullHTML = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>看房总报告 - ${Utils.today()}</title>
      <style>
        body{font-family:'PingFang SC','Microsoft YaHei',sans-serif;color:#1D1D1F;line-height:1.6;font-size:12px;max-width:900px;margin:0 auto;padding:20px;}
        h1{font-size:22px;color:#0071E3;text-align:center;border-bottom:2px solid #0071E3;padding-bottom:8px;}
        h2{font-size:15px;color:#0071E3;border-left:4px solid #0071E3;padding-left:8px;margin:20px 0 8px;}
        table{width:100%;border-collapse:collapse;margin:6px 0;}
